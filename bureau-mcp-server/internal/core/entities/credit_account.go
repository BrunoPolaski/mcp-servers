...
type CreditAccount struct {
	...
	// Account Details
	OpenedDate time.Time `gorm:"not null"`
	ClosedDate *time.Time
	Status     string `gorm:"size:50;not null;index"` // active, closed, charged_off, defaulted

	// Credit Limits & Balances
	CreditLimit     *float64 `gorm:"index"`
	CurrentBalance  float64  `gorm:"not null;index"`
